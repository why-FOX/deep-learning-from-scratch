...
Color = [[0, 0, 0],
                    ...
                    [128, 192, 0],
                    [0, 64, 128]]
def voc_ap(rec,prec,use_07_metric=False):#给一个 Precision-Recall 曲线，帮你计算曲线下面的面积（AP）
    if use_07_metric:
        # 11 point metric
        ap = 0.
...
if __name__ == '__main__':
    ...
    for line in lines:
        ...
        file_list.append(splited)#按空格存进filelist里
    f.close()
    ...
    for index,image_file in enumerate(file_list):
        ...
        for i in range(num_obj):
            ...
            target[(image_id,class_name)].append([x1,y1,x2,y2])#dataset里面的target（7*7*30）是用在loss文件里的
    #
    #start test
    #
    print('---start test---')
    ...
    model.cuda()
    count = 0
    for image_path in tqdm(image_list):#对所有要预测的图片列表
        result = predict_gpu(model,image_path,root_path='D:\github\deep-learning-from-scratch\VOC2012\JPEGImages\\') #result[[left_up,right_bottom,class_name,image_path],]
