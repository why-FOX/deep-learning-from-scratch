...
class yoloDataset(data.Dataset):
    ...
    def __init__(self,root,list_file,train,transform):
        ...
        if isinstance(list_file, list):
            # 拼接多个 list_file 文件内容，如果传入了多个文件的话
            tmp_file = r'D:\github\deep-learning-from-scratch\pytorch-YOLO-v1-master\pytorch-YOLO-v1-master\tmp\listfile.txt'
    ...
    def __getitem__(self,idx):
        ...
        img = cv2.imread(os.path.join(self.root,fname))#读取图像文件
        if img is None:
        ...
        # #debug
        h,w,_ = img.shape
        boxes /= torch.Tensor([w,h,w,h]).expand_as(boxes)#boxes.shape=[num.boxes,4]
    ...
    def encoder(self,boxes,labels):
        ...
        labels (tensor) [...]
        return 7x7x30 target
        '''
        grid_num = 14
        target = torch.zeros((grid_num,grid_num,30))
        cell_size = 1./grid_num
        wh = boxes[:,2:]-boxes[:,:2]      #shape=[num,2]
        cxcy = (boxes[:,2:]+boxes[:,:2])/2#shape=[num,2]
        for i in range(cxcy.size()[0]):
            cxcy_sample = cxcy[i]
            ij = (cxcy_sample/cell_size).ceil()-1 #找到网格的整数索引,向上取整再-1
            ...
            target[int(ij[1]),int(ij[0]),5:7] = delta_xy#真实的ground truth只有一个框
        return target
...
def main():
    ...
    print(f"是否成功加载：{len(train_dataset)} 个样本")
