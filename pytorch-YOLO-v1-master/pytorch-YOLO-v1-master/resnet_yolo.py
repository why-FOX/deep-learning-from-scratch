...
class detnet_bottleneck(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, block_type='A'):#A只有通道数不一致加，B强制加一个下采样以及shortcut卷积层
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1470):
        ...
        # self.layer5 = self._make_layer(block, 512, layers[3], stride=2)#如果是basicblock的话
        ...
        self.bn_end = nn.BatchNorm2d(30)#7*7*30输出
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        layers = []
        layers.append(block(self.inplanes, planes, stride, downsample))#planes * block.expansion输出就是这个只是隐式写成planes
